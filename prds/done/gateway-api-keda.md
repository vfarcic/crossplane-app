...
- [x] End-to-end validation on a real cluster provisioned by crossplane-kubernetes — deployed App XR (`silly-demo`) on control plane (`a-team` namespace, ProviderConfig `a-team`) targeting AKS destination cluster (`dev` namespace). Verified: all 6 Object CRs SYNCED+READY on control plane; Deployment, Service, HTTPRoute, ScaledObject, HTTPScaledObject in `dev`, ReferenceGrant in `keda` namespace on AKS; KEDA scaled to zero after cooldown; cold-start HTTP 200 with `x-keda-http-cold-start: true` header (~2.5s latency); Prometheus-based scale-up from 1→2 replicas at ~173 req/s (`ceil(173/100) = 2`). Bug found and fixed: Prometheus query used `_namespace` (XR namespace) instead of `_targetNamespace` (HTTPRoute namespace) — only affects deployments where `targetNamespace` differs from XR namespace. Issue found: Envoy Gateway PodMonitor on AKS missing `release: kube-prometheus-stack` label (Prometheus wasn't scraping Envoy metrics); manually patched for validation; needs feature request to crossplane-kubernetes
...
- [x] KCL: `_object` lambda — wraps manifest in `kubernetes.m.crossplane.io/v1alpha1 Object` with `providerConfigRef` (uses `kind: ProviderConfig` for namespace-scoped `.m.` API); sets namespace from `_targetNamespace`
...
- [x] Setup: Create namespace-scoped `ProviderConfig` with `InjectedIdentity` per test namespace (each chainsaw test applies `tests/common/provider-config.yaml`)
...
- [x] Bump major version (breaking change: `providerConfigName` now required) — v3.0.46 → v4.0.0 in `.semver.yaml` and `config.yaml`
...
| 2026-02-23 | Explicit App XR deletion and ProviderConfig finalizer patch in chainsaw `finally` blocks | Namespace-scoped Object CRs with provider-kubernetes finalizers cause deadlock during namespace deletion. The `finally` block runs BEFORE chainsaw's auto-cleanup, so it: (1) deletes the App XR, (2) waits for Object CRs to be gone, (3) deletes ProviderConfig with `--wait=false`, (4) patches `in-use.crossplane.io` finalizer off the ProviderConfig. Steps 3-4 are needed because the provider-kubernetes controller re-adds the finalizer after patching if Objects haven't fully cleared from its cache. Deleting and immediately patching prevents the race. Chainsaw's auto-cleanup then finds everything already gone | All chainsaw tests use `finally` block; `cleanup: 120s` timeout set as safety net for chainsaw's auto-deletion phase |
...
| 2026-02-24 | Switch from `ClusterProviderConfig` to namespace-scoped `ProviderConfig` | Real-world usage (e.g., AKS control plane) uses namespace-scoped `ProviderConfig` in the App XR's namespace, not cluster-scoped `ClusterProviderConfig`. Tests should match real usage. The `.m.` API requires `kind: ProviderConfig` explicitly in `providerConfigRef` | `_object` lambda sets `providerConfigRef.kind = "ProviderConfig"`; global `ClusterProviderConfig` setup removed (`scripts/provider-kubernetes.nu` is now no-op); each chainsaw test creates its own namespace-scoped `ProviderConfig` via `tests/common/provider-config.yaml`; `package/definition.yaml` description updated |
| 2026-02-24 | Prometheus query uses `_targetNamespace` instead of `_namespace` | Envoy names clusters based on HTTPRoute's actual namespace on the destination cluster, not the XR's namespace on the control plane. When `targetNamespace` differs (e.g., XR in `a-team`, resources in `dev`), the query scoped to the wrong namespace | KCL and generated composition updated; no impact on KinD tests (where `_targetNamespace` defaults to `_namespace`); all 3 chainsaw tests pass |
