...
- [ ] End-to-end validation on a real cluster provisioned by crossplane-kubernetes — AKS cluster available: control plane via `kubeconfig-cp.yaml` (Crossplane control plane with provider-kubernetes in `a-team` namespace), destination AKS cluster via `kubeconfig-aks.yaml` (has KEDA with HTTP Add-on, Envoy Gateway with Gateway `eg` at 40.71.234.92, Prometheus). A namespace-scoped `ProviderConfig` named `a-team` exists in the `a-team` namespace on the control plane. Deploy an App XR with scale-to-zero + gateway-api routing and verify cold-start handling works in a non-KinD environment; if issues found, either fix in crossplane-app or create a new feature request to crossplane-kubernetes
